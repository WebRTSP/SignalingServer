...
std::optional<std::string> DetectPublicIP(const std::string& stunServer)
{
    const auto& log = SignalingServerLog();

    g_autoptr(GError) error = nullptr;
    g_autoptr(GUri) uri = g_uri_parse(stunServer.c_str(), G_URI_FLAGS_HAS_PASSWORD, &error);
    if(!uri) {
        log->error("Failed to parse STUN server URL \"{}\": {}", stunServer, error->message);
        return {};
    }
